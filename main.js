...
function checkYearAdmission() {
  ...
  yearAdmission.addEventListener('input', function () {
    if (this.value != this.value.replace(/[^\+\d]/g, '')) {
      ...
      // alertYear.classList.toggle('is-active');
      yearAdmission.value = '';
    ...
    for (let i = 0; i < yearAdmission.value.length; i++) {
      if (yearAdmission.value[0] < 2 || yearAdmission.value[1] > 0 || (yearAdmission.value[2] >= 2 && yearAdmission.value[3] > 2)) {
        yearAdmission.disabled = true;
        alertYear2000.style.display = 'block';
        // alertYear2000.classList.toggle('is-active');
...
function fillForms() {
  ...
  if (
    ...
    course = dateAdmission + ' ' + '-' + ' ' + graduation + ' ' + 'Обучение закончено';
  }
...
function createListStudents() {
  ...
  returnButton.addEventListener('click', function () {
    ...
    fillForm.studentData.classList.toggle('is-active');
    buttonAdd.disabled = false;
  });
}
...
function letterValidate() {
  ...
  fillForm.studentData.classList.toggle('validate');
...
function validate() {
  ...
  fillForm.fieldName.forEach((elem) => {
    if (elem.value == '') {
      elem.classList.toggle('validate');
      letterValidate();
    }
  });
  return true;
